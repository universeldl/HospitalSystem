    ...
    <link rel="stylesheet" href="${pageContext.request.contextPath}/css/bootstrap-doctor-theme.css">
     ...
     <script src="${pageContext.request.contextPath}/js/patientUpdateInfo.js"></script>
             <script src="${pageContext.request.contextPath}/js/patientUpdatePwd.js"></script>
              <script src="${pageContext.request.contextPath}/js/getPatientForfeitInfo.js"></script>
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="collapse navbar-collapse main-navbar-collapse">
                        <a class="navbar-brand" href="${pageContext.request.contextPath}/patient.jsp"><strong>欢迎使用呼吸天使问卷管理系统</strong></a>
                        <ul class="nav navbar-nav navbar-right">
                          <s:if test="#session.patient!=null"><!-- 判断是否已经登录 -->
                            <li class="dropdown">
                                <a href="#" role="button" class="dropdown-toggle" data-hover="dropdown"> <i class="glyphicon glyphicon-user"></i> 欢迎您，<s:property value="#session.patient.name"/> <i class="caret"></i></a>
                                 ...
                                 <li><a href="#updateinfo" data-toggle="modal">个人资料</a></li>
                                    ...
                                    <li><a href="#updatepwd" data-toggle="modal">修改密码</a></li>
                                     <li role="presentation" class="divider"></li>
                                     <li><a href="${pageContext.request.contextPath}/patientLoginAction_logout.action">退出</a></li>
                                </ul>
    ...
    <div class="container">
        <!-- left, vertical navbar & content -->
           ...
           <div class="col-md-2 bootstrap-doctor-col-left">
            <ul class="nav navbar-collapse collapse bootstrap-doctor-navbar-side">
                <li>
                    <a href="${pageContext.request.contextPath}/surveyAction_findSurveyByPage.action"><i class="glyphicon glyphicon-chevron-right"></i> 问卷查询</a>
                </li>
                <s:if test="#session.patient!=null"><!-- 判断是否登录 -->
	                <li>
	                    <a href="${pageContext.request.contextPath}/patient/deliveryAction_findMyDeliveryInfoByPage.action"><i class="glyphicon glyphicon-chevron-right"></i> 分发信息</a>
	                </li>
                </s:if>
                 <s:if test="#session.patient!=null"><!-- 判断是否登录 -->
	                <li class="active">
	                    <a href="${pageContext.request.contextPath}/patient/forfeitAction_findMyForfeitInfoByPage.action"><i class="glyphicon glyphicon-chevron-right"></i> 逾期信息</a>
	                </li>
                </s:if>
                ...
                <div class="col-lg-12">
                    <div class="panel panel-default bootstrap-doctor-no-table-panel">
                        ...
                        <div class="bootstrap-doctor-no-table-panel-content bootstrap-doctor-panel-content collapse in">
                            <form class="form-horizontal" action="${pageContext.request.contextPath}/patient/forfeitAction_queryForfeitInfo.action" method="post">
                                <div class="row">
                               		 <div class="col-lg-5 form-group">
                                        <label class="col-lg-4 control-label" for="delivery_sno"><label class="text-danger"></label>分发编号</label>
                                        <div class="col-lg-8">
                                            <input class="form-control"  name="deliveryId" type="text" value="" placeholder="请输入分发编号">
                                            <label class="control-label" for="delivery_sno" style="display: none"></label>
                                        </div>
                                    </div>
                ...
                <div class="row">
                        ...
                        <table id="data_list" class="table table-hover table-bordered" cellspacing="0" width="100%">
                            ...
                            <s:iterator value="#request.pb.beanList" var="forfeit">
                             <tbody>
	                                ...
	                                <td><s:property value="#forfeit.deliveryInfo.penalty"/></td>
	                                <td>         
	                                	<button type="button" class="btn btn-info btn-xs" data-toggle="modal" data-target="#findRetrieveModal" onclick="getForfeitInfoById(<s:property value="#forfeit.deliveryId"/>)">查看</button>
                    ...
                    <s:if test="#request.pb!=null">
                        ...
                        <div class="pull-right"><!--右对齐--->
                           ...
                           <li><a href="${pageContext.request.contextPath}/patient/forfeitAction_${pb.url }pageCode=1">首页</a></li>
                           <li><a href="${pageContext.request.contextPath}/patient/forfeitAction_${pb.url }pageCode=${pb.pageCode-1 }">&laquo;</a></li><!-- 上一页 -->
                           <%-- 循环显示页码列表 --%>
								<c:forEach begin="${begin }" end="${end }" var="i">
								  ...
								  <%--如果是当前页则设置无法点击超链接 --%>
								  	...
								  	<c:otherwise>
								  		<li><a href="${pageContext.request.contextPath}/patient/forfeitAction_${pb.url }pageCode=${i}">${i}</a></li>
								  	</c:otherwise>
								  </c:choose>
								</c:forEach>
				        	   <%--如果当前页数没到总页数，即没到最后一页,则需要显示下一页 --%>
							  <c:if test="${pb.pageCode < pb.totalPage }">
								  <li><a href="${pageContext.request.contextPath}/patient/forfeitAction_${pb.url }pageCode=${pb.pageCode+1}">&raquo;</a></li>
							</c:if>
							<%--否则显示尾页 --%>
							<li><a href="${pageContext.request.contextPath}/patient/forfeitAction_${pb.url }pageCode=${pb.totalPage}">尾页</a></li>
							</ul>
                           </div>
                    </s:if>           
                    </div>
     ...
     <!--------------------------------------查看的模糊框------------------------>  
                                 <form class="form-horizontal">   <!--保证样式水平不混乱-->   
                                        <!-- 模态框（Modal） -->
									<div class="modal fade" id="findRetrieveModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
					...
					<div class="modal-dialog">
						<div class="modal-content">
							...
							<div class="form-group">
								...
								<div class="col-sm-7">
									<input type="text" class="form-control" id="name"  placeholder="请输入您的真实姓名" value='<s:property value="#session.patient.name"/>'>
							...
							<div class="form-group">
								...
								<div class="col-sm-7">
									<input type="text" class="form-control" id="phone"  placeholder="请输入您的联系号码" value='<s:property value="#session.patient.phone"/>'>
							...
							<div class="form-group">
								...
								<div class="col-sm-7">
									<input type="text" class="form-control" id="email"  placeholder="请输入您的邮箱" value='<s:property value="#session.patient.email"/>'>
