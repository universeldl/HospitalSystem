    ...
    <div class="container">
                ...
                <div class="col-lg-12">
                    <div class="panel panel-default bootstrap-doctor-no-table-panel">
                        ...
                        <div class="bootstrap-doctor-no-table-panel-content bootstrap-doctor-panel-content collapse in">
                            <form class="form-horizontal" action="${pageContext.request.contextPath}/doctor/retrieveManageAction_queryRetrieveInfo.action" method="post">
                                <div class="row">
                ...
                <div class="row">
                    ...
                    <s:if test="#request.pb!=null">
                        ...
                        <div class="pull-right"><!--右对齐--->
                           ...
                           <li><a href="${pageContext.request.contextPath}/doctor/retrieveManageAction_${pb.url }pageCode=1">首页</a></li>
                           <li><a href="${pageContext.request.contextPath}/doctor/retrieveManageAction_${pb.url }pageCode=${pb.pageCode-1 }">&laquo;</a></li><!-- 上一页 -->
                           <%-- 循环显示页码列表 --%>
								<c:forEach begin="${begin }" end="${end }" var="i">
								  ...
								  <%--如果是当前页则设置无法点击超链接 --%>
								  	...
								  	<c:otherwise>
								  		<li><a href="${pageContext.request.contextPath}/doctor/retrieveManageAction_${pb.url }pageCode=${i}">${i}</a></li>
								  	</c:otherwise>
								  </c:choose>
								</c:forEach>
				        	   <%--如果当前页数没到总页数，即没到最后一页,则需要显示下一页 --%>
							  <c:if test="${pb.pageCode < pb.totalPage }">
								  <li><a href="${pageContext.request.contextPath}/doctor/retrieveManageAction_${pb.url }pageCode=${pb.pageCode+1}">&raquo;</a></li>
							</c:if>
							<%--否则显示尾页 --%>
							<li><a href="${pageContext.request.contextPath}/doctor/retrieveManageAction_${pb.url }pageCode=${pb.totalPage}">尾页</a></li>
							</ul>
                           </div>
                    </s:if>           
                    </div>
     ...
     <!--------------------------------------查看答卷的模糊框------------------------>
                                 <form class="form-horizontal">   <!--保证样式水平不混乱-->
                                        <!-- 模态框（Modal） -->
									<div class="modal fade" id="findAnswerSheetModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
