    ...
    <link rel="stylesheet" href="${pageContext.request.contextPath}/css/bootstrap-doctor-theme.css">
        ...
        <script src="${pageContext.request.contextPath}/jQuery/jquery-3.1.1.min.js"></script>
            ...
            <script src="${pageContext.request.contextPath}/js/doctorUpdateInfo.js"></script>
 			...
 			<script src="${pageContext.request.contextPath}/js/deliverySurvey.js"></script>
    ...
    <div class="container">
                ...
                <div class="col-lg-12">
                    <div class="panel panel-default bootstrap-doctor-no-table-panel">
                        <div class="panel-heading">
                            <div class="text-muted bootstrap-doctor-box-title">发送问卷</div>
                        </div>
                        <div class="bootstrap-doctor-no-table-panel-content bootstrap-doctor-panel-content collapse in">
                            <form class="form-horizontal">
                                <div class="row">
                                    <div class="col-lg-5 form-group">
                                        <label class="col-lg-4 control-label" for="delivery_sno"><label class="text-danger">*&nbsp;</label>用户名</label>
                                        <div class="col-lg-8">
                                            <input class="form-control" id="deliveryPatientOpenID" name="deliveryPatientOpenID" type="text" value="" placeholder="请输入病人用户名">
                                            <label class="control-label" for="delivery_sno" style="display: none"></label>
                                        </div>
                                    </div>
                                    
                                    <div class="col-lg-5 form-group">
                                        <label class="col-lg-4 control-label" for="delivery_bno"><label class="text-danger">*&nbsp;</label>病人密码</label>
                                        <div class="col-lg-8">
                                            <input class="form-control" id="pwd" name="pwd" type="password" value="" placeholder="请病人输入密码">
                                            <label class="control-label" for="delivery_bno" style="display: none"></label>
                                        </div>
                                    </div>
                                    
                                    
                                    <div class="col-lg-2 form-group">
                                        <button type="button" class="btn btn-primary" id="btn_delivery">发送问卷</button>
                                    </div>
                                </div>
                            </form>
                        </div>
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <table id="data_list" class="table table-hover table-bordered" cellspacing="0" width="100%">
                            ...
                            <s:iterator value="#request.pb.beanList" var="delivery">
                             <tbody>
	                         	   <td><s:property value="#delivery.deliveryId"/></td>
	                                <td><s:property value="#delivery.survey.surveyName"/></td>
	                                <td><s:property value="#delivery.patient.openID"/></td>
	                                <td><s:property value="#delivery.patient.name"/></td>
	                                <td><s:date name="#delivery.deliveryDate" format="yyyy-MM-dd" /></td>
	                                <td><s:date name="#delivery.endDate" format="yyyy-MM-dd" /></td>
	                                <td>         
	                                	<button type="button" class="btn btn-info btn-xs" data-toggle="modal" data-target="#findDeliveryModal" onclick="getDeliveryInfoById(<s:property value="#delivery.deliveryId"/>)">查看</button>
                    ...
                    <s:if test="#request.pb!=null">
                        ...
                        <div class="pull-right"><!--右对齐--->
                           ...
                           <li><a href="${pageContext.request.contextPath}/doctor/deliveryManageAction_${pb.url }pageCode=1">首页</a></li>
                           <li><a href="${pageContext.request.contextPath}/doctor/deliveryManageAction_${pb.url }pageCode=${pb.pageCode-1 }">&laquo;</a></li><!-- 上一页 -->
                           <%-- 循环显示页码列表 --%>
								<c:forEach begin="${begin }" end="${end }" var="i">
								  ...
								  <%--如果是当前页则设置无法点击超链接 --%>
								  	...
								  	<c:otherwise>
								  		<li><a href="${pageContext.request.contextPath}/doctor/deliveryManageAction_${pb.url }pageCode=${i}">${i}</a></li>
								  	</c:otherwise>
								  </c:choose>
								</c:forEach>
				        	   <%--如果当前页数没到总页数，即没到最后一页,则需要显示下一页 --%>
							  <c:if test="${pb.pageCode < pb.totalPage }">
								  <li><a href="${pageContext.request.contextPath}/doctor/deliveryManageAction_${pb.url }pageCode=${pb.pageCode+1}">&raquo;</a></li>
							</c:if>
							<%--否则显示尾页 --%>
							<li><a href="${pageContext.request.contextPath}/doctor/deliveryManageAction_${pb.url }pageCode=${pb.totalPage}">尾页</a></li>
							</ul>
                           </div>
                    </s:if>           
                    </div>
     ...
     <!--------------------------------------查看的模糊框------------------------>  
                                 <form class="form-horizontal">   <!--保证样式水平不混乱-->   
                                        <!-- 模态框（Modal） -->
									<div class="modal fade" id="findDeliveryModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
